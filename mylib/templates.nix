{ lib, ... }: rec {
  ...
  /* haha, no.
  shareChroot = dir: closure: {
    RootDirectory = dir;
    BindReadOnlyPaths = [
      "/dev/log" "/run/systemd/journal/socket" "/run/systemd/journal/stdout"
    ] ++ closure;
  };
  chrootWith = dir: closure: {
    MountAPIVFS = "yes";
  } // shareChroot dir closure;
  closureBy = pkgs: packages: let
    rpkgs = builtins.map (x: "${x}") packages;
    rfile = pkgs.closureInfo { rootPaths = rpkgs; };
    lines = lib.splitString "\n" (builtins.readFile "${rfile}/store-paths");
    clean = lib.subtractLists [ "" ] lines ++ rpkgs;
    filthy = builtins.map (x: "${x}:${x}") clean;
  in lib.flatten filthy;
  */ 
  noManual = {
    html.enable = false;
    json.enable = false;
